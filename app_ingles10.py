...
def display_fill_in_the_blanks_exercise(level):
    ...
    # === CORREÇÃO: Exibição da frase do exercício ===
    st.markdown(f"### Complete a frase:")
    st.markdown(f"## **`{current['frase']}`**") # Adicionado para exibir a frase
    ...
    col1, col2, col3 = st.columns(3) # Adicionado mais uma coluna para a dica
    with col1:
    ...
    with col3: # Coluna para o botão de dica
        if st.button("💡 Dica de Gramática"):
            grammar_type = current['tipo']
            tip = grammar_tips.get(grammar_type, "Não há uma dica específica para este tipo de gramática.")
            st.info(f"**Dica sobre {grammar_type}:** {tip}")
